...
_topology.add_edge(0, 2, source=0, target=1, resources={'bw': 10000})
_topology.add_edge(2, 0, source=1, target=0, resources={'bw': 10000})
_topology = Topology(u'NoName', _topology)
#TODO: make sure the JSON sets the mboxes accordingly
_topology.set_mbox(1)
...
#        _topology = Topology.from_json(data)
        ...
        for s in _topology.nodes():
            ...
            for t in _topology.nodes():
                ...
                if s != t:
                    _paths[s][t] = list(generate_paths_ie(s, t, _topology, has_mbox_predicate, 100, 5, modify_func=use_mbox_modifier))

        logger.debug("PATHS: " + str(_paths))
